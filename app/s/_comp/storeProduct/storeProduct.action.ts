...
export const loadMoreStoreProductsAction = async ({
    ...
    cacheTag(
        hasSlugs
            ? `store-Products-section-${storeSlug}-${slugs.join("-")}-${cursor ? cursor : 0}`
    ...
    try {
        let context = null;
        if (hasSlugs) {
            context = await getCategoryContext(storeSlug, slugs);
        ...
        // console.error(error);
        throw new Error("Error while - load More Store Products Action ");
        // return {
        //   items: [],
        //   nextCursor: null,
        //   hasMore: false,
