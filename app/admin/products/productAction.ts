...
export const createProductAction = async (
    ...
    try {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            if (childCount > 0) {
                throw new Error(
                    "Products can only be added to leaf categories",
                );
            }
    ...
    // if (redirectId) {
    //     queueProductReindex(redirectId)
    // }
    ...
    if (admin?.id && categoryIdForRevalidate) {
        revalidateTag(
            `admin-category-products-${admin.id}-${categoryIdForRevalidate}`,
            "max",
        );
    }
...
export const createProductVariant = async (
    ...
    try {
        ...
        const createVariantSchema = z.object({
            ...
            stock: z.coerce
                .number()
                .int()
                .min(0, "Valid stock quantity is required"),
            ...
            images: z
                ...
                .refine(
                    (imgs) =>
                        imgs.reduce((sum, img) => sum + img.size, 0) <
                        10 * 1024 * 1024,
        ...
        await Promise.all(
            uploadedImages.map((id) => cloudinary.uploader.destroy(id)),
        );
        return {
            error:
                error instanceof Error
    ...
    if (productIdParam) {
        queueProductReindex(Number(productIdParam))
...
export const deleteProduct = async (productId: number) => {
    ...
    if (product?.adminId) {
        ...
        revalidateTag(
            `admin-category-products-${product.adminId}-${product.categoryId}`,
            "max",
        );
    }
    return { success: true, message: null };
